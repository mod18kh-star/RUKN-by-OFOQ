...
public sealed class ContentManagementService
{
    ...
    public async Task<IReadOnlyList<ContentPageResult>> GetPagesAsync(CancellationToken ct=default)
    {
        EnsureTenant();
        return (await _pages.GetAllAsync(ct)).OrderBy(x=>x.Title).Select(Map).ToArray();
    }

    public Task<ContentPageResult> CreatePageAsync(
        string title,
        string slug,
        string body,
        string? seoTitle,
    ...
    public async Task<ContentPageResult> CreatePageAsync(
        string title,
        string slug,
        string body,
        string? seoTitle,
        ...
        CancellationToken ct=default)
    {
        ...
        if(await _pages.SlugExistsAsync(slug.Trim().ToLowerInvariant(),cancellationToken:ct))
        {
            throw new ArgumentException("Page slug already exists.");
        }

        var now=_time.GetUtcNow();
        var page=ContentPage.Create(
    ...
    public Task<ContentPageResult?> UpdatePageAsync(
        ContentPageId id,
        string title,
        string slug,
        string body,
    ...
    public async Task<ContentPageResult?> UpdatePageAsync(
        ContentPageId id,
        string title,
        string slug,
        string body,
        ...
        CancellationToken ct=default)
    {
        ...
        if(await _pages.SlugExistsAsync(slug.Trim().ToLowerInvariant(),id,ct))
        {
            throw new ArgumentException("Page slug already exists.");
        }
        ...
        if(publish) page.Publish(now,actor); else page.Unpublish(now,actor);
        await _uow.SaveChangesAsync(ct);
        return Map(page);
    }

    public async Task<bool> DeletePageAsync(ContentPageId id,CancellationToken ct=default)
    {
        EnsureTenant();
        var page=await _pages.GetByIdAsync(id,ct);
        if(page is null)return false;
        var nav=(await _navigation.GetAllAsync(ct)).Any(x=>x.TargetType==NavigationTargetType.Page&&x.TargetId==id.Value);
        if(nav)throw new InvalidOperationException("Remove page navigation links before deleting the page.");
        _pages.Remove(page);
        await _uow.SaveChangesAsync(ct);
        return true;
    }

    public async Task<IReadOnlyList<NavigationItemResult>> GetNavigationAsync(CancellationToken ct=default)
    {
        EnsureTenant();
        return (await _navigation.GetAllAsync(ct)).OrderBy(x=>x.Location).ThenBy(x=>x.ParentItemId.HasValue).ThenBy(x=>x.SortOrder).Select(Map).ToArray();
    }
    ...
    public async Task<bool> DeleteNavigationAsync(NavigationItemId id,CancellationToken ct=default)
    { EnsureTenant();var item=await _navigation.GetByIdAsync(id,ct);if(item is null)return false;if((await _navigation.GetAllAsync(ct)).Any(x=>x.ParentItemId==id))throw new InvalidOperationException("Delete child navigation items first.");_navigation.Remove(item);await _uow.SaveChangesAsync(ct);return true;}
    ...
    private static ContentPageResult Map(ContentPage x)=>new(
        x.Id.Value,
        x.Title,
        x.Slug,
        x.Body,
