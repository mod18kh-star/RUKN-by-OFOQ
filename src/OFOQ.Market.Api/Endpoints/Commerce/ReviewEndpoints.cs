...
public static class ReviewEndpoints
{
 public static IEndpointRouteBuilder MapReviewEndpoints(this IEndpointRouteBuilder e)
 {
  ...
  var sf=e.MapGroup("/api/storefront/{storeSlug}").WithTags("Storefront Reviews"); sf.MapGet("/products/{productSlug}/reviews",PublicReviewsAsync);sf.MapGet("/reviews",PublicStoreReviewsAsync);sf.MapGet("/trust-metrics",TrustMetricsAsync);return e;
 }
 ...
 private static async Task<IResult> PublicStoreReviewsAsync(string storeSlug,int? take,IReviewQueryRepository q,CancellationToken ct){try{var x=await q.GetPublicStoreReviewsAsync(storeSlug,take??24,ct);return x is null?Results.NotFound():Results.Ok(new PublicStoreReviewPageResponse(x.AverageRating,x.ReviewCount,x.Reviews.Select(r=>new PublicStoreReviewResponse(r.Id,r.ProductId,r.ProductName,r.ProductSlug,r.Rating,r.Body,r.IsVerifiedPurchase,r.MerchantReply,r.CreatedAtUtc)).ToArray()));}catch(ArgumentException ex){return Bad("review_query_invalid",ex.Message);} }
